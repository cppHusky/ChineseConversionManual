\section{\content{注音符號、漢語拼音簡介}{汉语拼音、注音符号简介}}\label{section:注音符號、漢語拼音簡介}
\subsection{\content{背景知識}{背景知识}}\label{subsection:背景知識}
...
\content{我們判斷自己是否「認識」某個字，往往也要以「是否會唸」作為依據。舉些例子，「\textbf{柃}木」「\textbf{囿}於」「虎\textbf{兕}」「\textbf{朊}粒」「\textbf{旻}天」中的五個粗體字，簡單到只要让人看過一眼就會寫了，但是你真的認得它們嗎？與之相反，對於「食\textbf{鹽}」「\textbf{黎}明」「\textbf{殷}切」「\textbf{贏}家」中的四个粗體字，我們未必都會寫（或許正是人們提筆忘字的範例），但是我們只要看上一眼就可以確信，我們認識這些字！}{我们判断自己是否“认识”某个字，往往也要以“是否会念”作为依据。举些例子，“\textbf{柃}木”“\textbf{囿}于”“虎\textbf{兕}”“\textbf{朊}粒”“\textbf{旻}天”中的五个粗体字，简单到只要让人看过一眼就会写了，但是你真的认得它们吗？与之相反，对于“扫\textbf{帚}”“\textbf{黎}明”“\textbf{殷}切”“\textbf{赢}家”中的四个粗体字，我们未必都会写（或许正是人们提笔忘字的范例），但是我们只要看上一眼就可以确信，我们认识这些字！}\par
...
\subsection{\content{音節的構成}{音节的构成}}\label{subsection:音節的構成}
...
\content{漢字是單音節的\footnote{漢語本身不是單音節的。例如，「咖啡」一詞有兩個音節。這個詞就是語義上的最小單位，我們不可能把「咖」「啡」二字拆開來分別解釋它們的意思。但是在為漢字注音時，我們仍要以字為單位。}。對於絕大多數漢字來說\footnote{總是有些特例，比如作為計量單位的「瓩」和「嗧」就是雙音節的漢字，分別讀作\bopin{ㄑㄧㄢ ㄨㄚˇ}{\pinyin{qian1wa3}}和\bopin{ㄐㄧㄚ ㄌㄨㄣˊ}{\pinyin{jia1lun2}}。這些字在港澳台仍然使用；而在中國大陸，它們多半隨著1977年\href{http://www.moe.gov.cn/ewebeditor/uploadfile/2015/01/12/20150112161542547.pdf}{〈關於部分計量單位名稱統一用字的通知〉}被廢除。}，一個字就是一個音節，也就是一個發音單位；而像英語這樣的語言則不同，有些單詞是單音節的，有些單詞則是多音節的。}{汉字\footnote{汉语本身不是单音节的。例如，“咖啡”一词有两个音节。这个词就是语义上的最小单位，我们不可能把“咖”“啡”二字拆开来分别解释它们的意思。但是在为汉字注音时，我们仍要以字为单位。}是单音节的。对于绝大多数汉字来说\footnote{总是有些特例，比如“瓩”和“嗧”就是双音节的汉字，分别读作\bopin{ㄑㄧㄢ ㄨㄚˇ}{\pinyin{qian1wa3}}和\bopin{ㄐㄧㄚ ㄌㄨㄣˊ}{\pinyin{jia1lun2}}。这些字在中国大陆多半随著1977年\href{http://www.moe.gov.cn/ewebeditor/uploadfile/2015/01/12/20150112161542547.pdf}{《关于部分计量单位名称统一用字的通知》}被废除，而在港澳台仍然使用。}，一个字就是一个一个音节，也就是一个发音单位；而像英语这样的语言则不同，有些单词是单音节的，有些单词则是多音节的。}\par
...
\subsection{\content{由漢語拼音推導注音符號}{由汉语拼音推导注音符号}}\label{subsection:由漢語拼音推導注音符號}
...
\begin{figure}[H]
    \NewDocumentCommand{\drawkey}{mmmm}{
        \begin{scope}[shift={#1}]
            ...
            \draw (0,-.82)node[anchor=base west]{\ttfamily\small#4};
        \end{scope}
    }
...
\begin{table}[H]
    ...
    \begin{NiceTabular}{|*{6}{>{\columncolor{blue!7.5}}c>{\columncolor{red!7.5}}c|}}
        ...
        ㄈ & f & ㄌ & l & ㄏ & h & ㄒ & x & ㄖ & r & ㄙ & s \\\bottomrule
    \end{NiceTabular}
\end{table}
...
\begin{table}[H]
    ...
    \CodeBefore
        \columncolor{blue!7.5}{1}
        \columncolor{red!7.5}{2-3}
...
\begin{table}[H]
    ...
    \begin{NiceTabular}{|c|cc|cc|}[corners,hlines]
    \CodeBefore
        \rowcolor{lightgray}{1}
...
\subsection{\content{由注音符號推導漢語拼音}{由注音符号推导汉语拼音}}\label{subsection:由注音符號拄導漢語拼音}
